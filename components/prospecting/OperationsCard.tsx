    ...
    <section className="glass-card flex h-full flex-col rounded-3xl p-5 sm:p-6">
      ...
      <div className="mt-3 flex flex-1 items-center gap-4">
        ...
        <button
          ...
          className="group relative grid h-28 w-28 shrink-0 place-items-center rounded-full sm:h-36 sm:w-36"
        >
          {/* Anel externo metálico */}
          <span
            className="absolute inset-0 rounded-full"
            style={{
              background: "conic-gradient(from 180deg, oklch(0.92 0.01 255), oklch(0.78 0.02 255), oklch(0.96 0.005 255), oklch(0.82 0.02 255), oklch(0.92 0.01 255))",
              boxShadow: "0 10px 30px -10px oklch(0.45 0.05 255 / 0.4), 0 1px 0 0 oklch(1 0 0) inset",
            }}
            aria-hidden="true"
          />
          {/* Sulco interno */}
          <span
            ...
            style={{
              background: "linear-gradient(180deg, oklch(0.97 0.004 255), oklch(0.88 0.01 255))",
              boxShadow: "0 -2px 6px oklch(1 0 0) inset, 0 4px 10px oklch(0.5 0.05 255 / 0.25) inset",
            }}
            aria-hidden="true"
          />
          {/* Núcleo azul brilhante */}
          <span
            className={`absolute inset-[22px] grid place-items-center rounded-full text-primary-foreground transition-transform duration-200 group-hover:scale-[1.04] group-active:scale-95 ${
              isRunning ? "" : ""
            }`}
            style={{
              ...
              boxShadow: "0 1px 0 0 oklch(1 0 0 / 0.5) inset, 0 -3px 8px oklch(0.3 0.1 258 / 0.4) inset, 0 8px 18px -6px oklch(0.52 0.22 258 / 0.6)",
            }}
            aria-hidden="true"
          >
            {isRunning ? (
              ...
              <svg width="38" height="38" viewBox="0 0 24 24" fill="currentColor" className="ml-1">
                <polygon points="6 4 20 12 6 20 6 4" />
              </svg>
            )}
          </span>
          {/* Pulso quando rodando */}
          {isRunning && (
            <span className="absolute inset-0 animate-ping rounded-full border-2 border-primary/40" aria-hidden="true" />
          )}
        </button>
      </div>
